        ...
        messages=[
            {
                ...
                "content": [
                    ...
                    {"type": "text", "text": "告诉我图片中服装的以下特征，只要答案，格式为json字符串，颜色四季型（春季型/夏季型/秋季型/冬季型，可以多选）适合年龄范围 直得分 曲得分 直曲总分（直得分-曲得分） 大量感得分 小量感得分 量感总分（大量感得分-小量感得）立体度（就是与面部风格立体度高的人的适配程度，总分十分）适合体型（H型/​X型/A型/O型/T型，可以多选）​​适合场景（职业/商务、休闲/日常、隆重社交） 特点（显高、显瘦、显气场、 显腰线 、显腿长、显胸大、显胸小）以及10个最能概括衣服特征的标签"},
                ],
            }
        ],
    )
...
def checkKey(key, bak_key, data, bak_value):
    if key in data:
        if isinstance(bak_value, (int, float)):
...
def GetPicDesc(img_url, shen_gao='162', ti_zhong='52', sex='女'):
    response = client.chat.completions.create(
